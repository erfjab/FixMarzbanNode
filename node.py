...
class ReSTXRayNode:
    ...
    def connected(self):
        ...
        if (self._connected_cache is not None and 
            current_time - self._connected_cache_time < self._cache_ttl):
            return self._connected_cache
        
        try:
            self.make_request("/ping", timeout=30)
            logger.info("Node %s is connected", self.address)
            self._connected_cache = True
            self._connected_cache_time = current_time
            return True
        except NodeAPIError:
            logger.info("Node %s is not connected", self.address)
    ...
    def started(self):
        current_time = time.time()
        if (self._started_cache is not None and 
            current_time - self._started_cache_time < self._cache_ttl):
            return self._started_cache
        
        try:
            res = self.make_request("/", timeout=30)
            started_status = res.get("started", False)
            logger.info("Node %s started status: %s", self.address, started_status)
            self._started_cache = started_status
            self._started_cache_time = current_time
            return started_status
        except NodeAPIError:
            self._started_cache = False
            self._started_cache_time = current_time
            logger.info("Node %s is not started", self.address)
            return False
